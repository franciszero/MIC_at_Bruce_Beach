...
ds18 = fo.load_dataset('YOLOv8x_BB158_500_t16_b')
ds31 = fo.load_dataset('YOLOv8x_BB426_760_t31_b')
for i, (s18, s31) in enumerate(zip(ds18, ds31)):
    if s18.filename == s31.filename:
        idx = test.index.values.size
        test.loc[idx, "Image"] = s31.filename
        if s18.detections is None and s31.detections is None:
            test.loc[idx, "ground truth"] = 0
        else:
            gt16 = s18.detections.detections
            gt31 = s31.detections.detections
            if len(gt16) == len(gt31):
                test.loc[idx, "ground truth"] = len(gt16)
            else:
                print(s18.filename, len(gt16), " != ", len(gt31))

        pred16 = s18.predictions.detections
        if pred16 is None:
        ...
        print(s18.filename, s31.filename, )
    else:
        print(s18.filename,
              len(s18.detections.detections),
              len(s18.predictions.detections),
              s31.filename,

...
test = pd.DataFrame(columns=["filename", "ground_truth", "train18", "train31"], dtype=int, )
...
test_sort = test.sort_values(by=['ground_truth', 'train31', 'train18'], ascending=False)
tmp = test_sort.set_index(['filename']).stack()
tmp = tmp.rename_axis(index=['filename', 'class'])
tmp.name = 'value'
tmp = tmp.reset_index()

fig, ax = plt.subplots(1, 1, figsize=(10, 4), dpi=200)
sns.lineplot(tmp, x=tmp['filename'], y=tmp['value'], hue=tmp['class'],
...
plt.tight_layout()
# plt.show()
plt.savefig('./data/img_class_ground_truth/1.jpg')

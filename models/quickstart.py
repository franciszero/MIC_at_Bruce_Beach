...
class YOLOv8:
    ...
    def cnt_person(self, filename, save_file=False):
        if filename.endswith("avif"):  # or filename.endswith("png"):
            return -1
        results = self.model.predict(self.input + '/' + filename, verbose=False)
        cnt = 0
        bboxes = None
        for result in results:
            ...
            for bbox in result.boxes:
                if int(bbox.cls.numpy()[0]) == 0:
                    cnt += 1
        if save_file:
            self.__save_result(cnt, filename, results[0])
        return cnt
...
if __name__ == '__main__':
    foo = Driver(i='../data/quickstart/test_data/', o='../data/quickstart/test_results/')
    dic = foo.scanImages(overwrite=True)
